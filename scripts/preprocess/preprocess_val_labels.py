...
image_dir = 'datasets/tusimple/images'  # 원본 이미지 경로
val_image_dir = 'datasets/val/images'   # Validation 이미지 경로
val_label_dir = 'datasets/val/labels'   # YOLO 형식 라벨 파일을 저장할 경로

# 출력 디렉토리 생성
os.makedirs(val_label_dir, exist_ok=True)

# TuSimple JSON 데이터를 YOLO 폴리곤 형식으로 변환하는 함수
def convert_label(json_file, output_dir, image_dir):
    ...
    with open(json_file, 'r') as f:
        for line in f:
            ...
            # 기존의 경로에서 'clips/' 부분 제거 및 폴더 구조를 "_"로 결합하여 새로운 파일명 생성
            relative_path = raw_file.replace('clips/', '')
            folder_structure = relative_path.replace(os.sep, '_').replace('/', '_')  # 폴더 구조를 '_'로 결합
            new_image_filename = folder_structure  # 새로운 파일명 생성
            ...
            label_filename = new_image_filename.replace('.jpg', '.txt')
...
if __name__ == '__main__':
    ...
    for label_file in label_files:
        ...
        convert_label(json_path, val_label_dir, val_image_dir)

    print(f"Converted {len(label_files)} label files to YOLO segmentation format.")
